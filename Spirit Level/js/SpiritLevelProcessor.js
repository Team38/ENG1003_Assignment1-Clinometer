...
function SpiritLevelProcessor() {
    ...
    function handleMotion(event) {
        ...
        if (swapClick % 2 === 0) {
            ...
            retval = (movingMedian(bufferRecord, rawMotionData));
        }
    }
    ...
    function displayAngle(x, y, z) {
        ...
        var displayRef = document.getElementById("message-area"),
            finalAngle = Math.acos(z / (Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2) + Math.pow(z, 2)))) * 180 / Math.PI;

        displayRef.innerHTML = finalAngle.toFixed(2) + "&deg from the Z-Axis";
